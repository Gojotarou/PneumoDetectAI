...
class Notification(db.Model):
    ...
    analysis = db.relationship('Analysis', backref=db.backref('notifications', cascade='all, delete-orphan'), lazy=True)
    ...
    def to_dict(self):
        return {
            ...
            'patient_id': self.patient_id,
            'patient_name': self.patient.name if self.patient else None,
            'patient_medical_id': self.patient.medical_id if self.patient else None,
            'analysis_id': self.analysis_id,
            'urgency_level': self.urgency_level,
            'is_read': self.is_read,
            'is_dismissed': self.is_dismissed,
            'is_acknowledged': self.is_acknowledged,
            'acknowledged_at': self.acknowledged_at.isoformat() + '+00:00' if self.acknowledged_at else None,
            'created_at': self.created_at.isoformat() + '+00:00' if self.created_at else None,
            'read_at': self.read_at.isoformat() + '+00:00' if self.read_at else None
        }
